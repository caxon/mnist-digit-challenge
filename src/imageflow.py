...
class Flow(object):
	...
	def __init__(self, array_or_filepath, squeeze=True):
	...
	def highlights(self, xys, fill=None):
		...
		for xy in xys:
			self.highlight( xy, fill)

	@chain
	def drawrect(self, rcrc, outline=None):
		x,y,xx,yy = rcrc
		xyxy = y,x,yy,xx
		img = imn(self.img)
		drawer = ImageDraw.Draw(img)
		drawer.rectangle(xyxy, outline=outline)
		self.img = np.array(img)

	@chain
	def drawrects(self, xyxys, outline= None):
		for xyxy in xyxys:
			self.drawrect(xyxy, outline)

	@chain
	def calculate_regions(self, *, connectivity=2):
	...
	@chain
	def do(self, img_lambda):
		self.img = img_lambda(self.img)
	...
	def extract(self, reshape=False):
		# if idx is not None:
		# 	print(idx)
		""" get focal regions"""
		...
		for x0, y0, x1, y1 in self.regions:
			focal_regions.append(Flow(src_img[x0:x1, y0:y1]).array())
		if reshape:
			return np.expand_dims(np.stack(focal_regions,axis=0), 3)
	...
	def get(self):
		""" get img ndarray with original size"""
		if self.img.shape != self.original_shape:
			return np.reshape(self.img, self.original_shape)
		return self.img
